#!/usr/bin/env python3
"""
FIRMARBITER command-line runner.
...
Examples:
    ./python run_firmarbiter.py --firmware /path/to/corpus
    ./python run_firmarbiter.py --firmware image.zip --candidates firmae
    ./python run_firmarbiter.py --firmware corpus/ --candidates firmae,firmadyne
    ./python run_firmarbiter.py --firmware corpus/ --experiment-id pilot-01 --dry-run
...
class CliError(RuntimeError):
    """Raised for a user-facing FIRMARBITER CLI error."""
...
def scan_firmware(path_value: str) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(source.rglob("*")):
        ...
        if case["sha256"] in seen_hashes:
            print(
                f"[FIRMARBITER] Skipping duplicate bytes: {candidate}"
            )
            continue
...
def milestone_lines(
    ...
    for event in events:
        ...
        if event_name == "adapter_started":
            lines.append("[FIRMARBITER] ✓ Adapter started")

        elif event_name == "candidate_started":
            lines.append(
                "[FIRMARBITER] → Candidate execution started"
            )
            ...
            lines.append(
                f"[FIRMARBITER] {symbol} Stage {stage} {outcome}"
                f"{duration}{detail}"
            )
            ...
            if isinstance(endpoint, dict):
                ...
                lines.append(
                    "[FIRMARBITER] → Candidate endpoint claim: "
                    f"{protocol}://{host}:{port}"
                )

        elif event_name == "shutdown_started":
            lines.append("[FIRMARBITER] → Controlled shutdown started")

        elif event_name == "cleanup_complete":
            lines.append("[FIRMARBITER] ✓ Adapter cleanup completed")

        elif event_name == "adapter_stopped":
            outcome = event.get("outcome", "unknown")
            symbol = "✓" if outcome == "completed" else "✗"
            lines.append(
                f"[FIRMARBITER] {symbol} Adapter stopped: {outcome}"
            )
...
def execute_with_live_progress(
    *,
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=1,
        thread_name_prefix="firmarbiter-run",
    ) as executor:
        ...
        while not future.done():
            ...
            if (
                ...
                print()
                print(
                    f"[FIRMARBITER] Adapter error: {code}: {message}",
                    file=sys.stderr,
                )
                print(
                    "[FIRMARBITER] Controlled shutdown requested immediately.",
                    file=sys.stderr,
                )
...
def warn_about_emulated_host() -> None:
    ...
    if machine in {"aarch64", "arm64"}:
        print(
            "[FIRMARBITER] NOTE: this ARM64 host can be used for functional "
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "FIRMARBITER — candidate-neutral firmware benchmark runner"
        ),
...
Examples:
  ./python run_firmarbiter.py --firmware /path/to/corpus
  ./python run_firmarbiter.py --firmware image.zip --candidates firmae
  ./python run_firmarbiter.py --firmware corpus --candidate firmae
  ./python run_firmarbiter.py --firmware corpus --experiment-id pilot-01 --dry-run
  ./python run_firmarbiter.py --list-candidates
        """,
    )
...
# pattern used by fact_extractor/ and emba/ (the adapters actually
# built from this template) — you should not need to change them.
# What you DO need to add: installing your actual candidate tool and
...
def main() -> int:
    ...
    try:
        ...
        print(f"[FIRMARBITER] ERROR: {exc}", file=sys.stderr)
        return 2

    total_runs = len(cases) * len(selected_adapters)

    print()
    print(f"[FIRMARBITER] Experiment      : {experiment_id}")
    print(f"[FIRMARBITER] Firmware images : {len(cases)}")
    print(
        "[FIRMARBITER] Candidates      : "
    ...
    for case in cases:
        for adapter_id, adapter in selected_adapters.items():
            ...
            if run_directory.exists():
                print(
                    f"[FIRMARBITER] ERROR {adapter_id} / "
            ...
            try:
                ...
                coordinator_errors += 1
                print(
                    f"[FIRMARBITER] COORDINATOR ERROR — {adapter_id} / "
                    f"{case['case_id']}: {exc}",
                    file=sys.stderr,
                )
    ...
    print()
    print(
        "[FIRMARBITER] Aggregate this experiment with:\n"
