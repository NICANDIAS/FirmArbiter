...
DOCKER_BACKEND_CAPABILITIES: dict[str, RequirementStatus] = {
    ...
    "nested-containers": RequirementStatus(
        ...
        "whether the candidate's own container was removed. The "
        "docker:dind image is now pinned by digest (was an unpinned "
        "tag, a real gap against this project's own reproducibility "
        "standard, closed 2026-09-19). A real child container has now "
        "been created and run INSIDE the nested daemon and confirmed "
        "end-to-end -- not just daemon reachability -- via "
        "tools/manual_test_nested_containers.py (2026-09-20): a fresh "
        "image was pulled inside the sidecar, run, and its own real "
        "output (including its own hostname) was captured back on the "
        "host, then everything was torn down cleanly. This was "
        "previously the single most-cited gap in this note; it's "
        "closed for the core mechanism now, with two real limits on "
        "what that proves: only one simple, unprivileged, lightweight "
        "image with a trivial command was tested -- not a real "
        "candidate's actual workload (larger images, long-running "
        "processes, anything needing --privileged inside the nested "
        "daemon itself) -- and the test suite's own automated mock "
        "adapter (tests/fixtures/docker-adapters/mock-nested-"
        "containers-adapter/) still only proves `docker version` "
        "reachability, not a real `container create`; the manual "
        "script above is not yet part of the automated suite. "
        "Still genuinely open, not independently re-checked "
