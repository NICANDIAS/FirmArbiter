...
#
# Translates FIRMARBITER's standard call into your firmware-security pipeline
# invocation.
#
# FIRMARBITER always calls adapters like this:
...
if [ -z "$FIRMWARE_PATH" ] || [ -z "$ARCHITECTURE" ]; then
    echo "[FIRMARBITER][your_pipeline] ERROR: usage: run_adapter.sh <firmware_path> <architecture>" >&2
    exit 3
fi

if [ ! -f "$FIRMWARE_PATH" ]; then
    echo "[FIRMARBITER][your_pipeline] ERROR: firmware file not found: $FIRMWARE_PATH" >&2
    exit 3
fi
...
if [ ! -f "$PIPELINE_DIR/pipeline/orchestrator/fw_baseline.py" ]; then
    echo "[FIRMARBITER][your_pipeline] ERROR: fw_baseline.py not found at $PIPELINE_DIR/pipeline/orchestrator/" >&2
    echo "[FIRMARBITER][your_pipeline] Set the PIPELINE_DIR environment variable to the correct path." >&2
    exit 2
fi

echo "[FIRMARBITER][your_pipeline] Starting your pipeline on: $(basename "$FIRMWARE_PATH") ($ARCHITECTURE)"
echo "[FIRMARBITER][your_pipeline] Using pipeline at: $PIPELINE_DIR"
...
echo "[FIRMARBITER][your_pipeline] Pipeline exited with code: $EXIT_CODE"
exit $EXIT_CODE
