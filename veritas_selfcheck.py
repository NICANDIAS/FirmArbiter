"""
veritas_selfcheck.py
--------------------
Runs automatically every time VERITAS starts. Checks that all Python
...
This module exists because one of the core criticisms VERITAS makes of other
firmware tools is that they have undocumented or outdated dependencies that
break silently. VERITAS holds itself to the same standard it applies to others.
...
This module is imported and called by run_veritas.py at startup. You can
also run it directly to check your environment without starting a benchmark:

    python veritas_selfcheck.py
"""
...
#
# When you add a new import to any VERITAS module, add a corresponding
...
# These are tools VERITAS calls via subprocess — if they are missing,
# certain probes will silently fail without this check.

# Docker is checked separately below with a detailed message because
# it is central to VERITAS's isolation model, not just an optional tool.
...
def _pip_cmd() -> list:
    ...
    and rejects direct pip installs. VERITAS always runs inside its own
...
def check_running_in_venv() -> bool:
    """
    Warn if VERITAS is not running inside its own virtual environment.
    ...
    if not in_venv:
        print("[VERITAS] Warning: not running inside the VERITAS virtual environment.")
        print("[VERITAS]          Run setup.sh first, then use ./python instead of python3.")
        print("[VERITAS]          Commands:")
        print("[VERITAS]            bash setup.sh")
        print("[VERITAS]            ./python run_veritas.py --list-candidates")
    return in_venv
...
def check_python_version() -> bool:
    ...
    if major < 3 or (major == 3 and minor < 10):
        print(f"[VERITAS] ERROR: Python 3.10 or higher is required.")
        print(f"[VERITAS]        You are running Python {major}.{minor}.")
        print(f"[VERITAS]        Install a newer Python: sudo apt install python3.12")
        return False
    return True
...
def check_and_update_packages(auto_update: bool = True) -> bool:
    ...
    for import_name, pip_name, min_version in REQUIRED_PACKAGES:
        ...
        if installed is None:
            ...
            print(f"[VERITAS] Missing package: {pip_name} — installing...")
            if auto_update and _install_package(pip_name):
                print(f"[VERITAS]   Installed {pip_name} successfully.")
            else:
                print(f"[VERITAS]   Could not install {pip_name} automatically.")
                print(f"[VERITAS]   Run manually: pip install {pip_name}")
                all_ok = False
            continue
        ...
        if installed < min_version:
            ...
            print(f"[VERITAS] Outdated: {pip_name} "
                  f"(installed {installed_str}, need >={required_str}) — upgrading...")
            if auto_update and _upgrade_package(pip_name):
                print(f"[VERITAS]   Upgraded {pip_name} successfully.")
            else:
                print(f"[VERITAS]   Could not upgrade {pip_name} automatically.")
                print(f"[VERITAS]   Run manually: pip install --upgrade {pip_name}")
                all_ok = False
...
def check_system_tools() -> bool:
    ...
    for tool, hint in REQUIRED_SYSTEM_TOOLS:
        if shutil.which(tool) is None:
            print(f"[VERITAS] Missing system tool: {tool}")
            print(f"[VERITAS]   Install with: {hint}")
            all_ok = False

    for tool, hint in OPTIONAL_SYSTEM_TOOLS:
        if shutil.which(tool) is None:
            print(f"[VERITAS] Optional tool not found: {tool} "
...
def check_docker() -> bool:
    ...
    if shutil.which("docker") is None:
        print("[VERITAS] Docker is not installed.")
        print("[VERITAS] Docker is required for candidate isolation.")
        print("[VERITAS] Install it from: https://docs.docker.com/engine/install/ubuntu/")
        return False
    ...
    try:
        ...
        if result.returncode != 0:
            print("[VERITAS] Docker is installed but the daemon is not running.")
            print("[VERITAS] Start it with: sudo systemctl start docker")
            print("[VERITAS] Enable on boot: sudo systemctl enable docker")
            return False
    except (subprocess.TimeoutExpired, FileNotFoundError):
        print("[VERITAS] Could not connect to Docker daemon.")
        return False
...
def check_schema_files() -> bool:
    ...
    if not schema.exists():
        print(f"[VERITAS] Warning: schemas/result_schema.json not found. "
...
def run_selfcheck(auto_update: bool = True, silent_if_ok: bool = True) -> bool:
    """
    Run all self-checks. Called automatically by run_veritas.py at startup.
    ...
    if all_ok and silent_if_ok:
        ...
        print("[VERITAS] Self-check passed. Environment is ready.")
...
if __name__ == "__main__":
    print("\n[VERITAS] Running environment self-check...\n")

    ok = run_selfcheck(auto_update=True, silent_if_ok=False)

    if ok:
        print("\n[VERITAS] All checks passed. VERITAS is ready to run.")
    else:
        print("\n[VERITAS] Some checks failed. Fix the issues above before running.")
        sys.exit(1)
