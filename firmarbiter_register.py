"""
firmarbiter_register.py
-------------------
Handles automatic registration of candidate tools in FIRMARBITER.

Supports two source modes for Docker image building:

  git mode   — FIRMARBITER clones the tool from a Git repository URL when
  ...
  local mode — FIRMARBITER copies the tool from a folder on your host machine
...
When you run --register-tool, FIRMARBITER asks which mode you want. Your
...
Usage:
    python run_firmarbiter.py --register-tool /home/ubuntu/Desktop/FirmAE
    python run_firmarbiter.py --register-tool /home/ubuntu/firmadyne
    python run_firmarbiter.py --register-tool /home/ubuntu/emba
    python run_firmarbiter.py --register-tool /home/ubuntu/firmware-security

Standalone:
    python firmarbiter_register.py /path/to/tool_folder
"""
...
RUN if [ "$TOOL_SOURCE" = "git" ]; then \\
        ...
        cd tool && git rev-parse HEAD > .firmarbiter_commit; \\
    fi"""
...
RUN if [ "$TOOL_SOURCE" = "local" ]; then \\
        ...
        if [ -d /opt/tool/.git ]; then \\
            cd /opt/tool && git rev-parse HEAD > .firmarbiter_commit 2>/dev/null || \\
            echo "local-copy-no-git" > /opt/tool/.firmarbiter_commit; \\
        else \\
            echo "local-copy-no-git" > /opt/tool/.firmarbiter_commit; \\
        fi; \\
    fi"""
...
# Auto-generated by FIRMARBITER for {tool_name}
...
# run_adapter.sh — auto-generated by FIRMARBITER for {tool_name}
# Runs inside the Docker container. Called by FIRMARBITER as:
...
if [ -z "$FIRMWARE" ] || [ -z "$ARCH" ]; then
    echo "[FIRMARBITER][{tool_name}] ERROR: usage: run_adapter.sh <firmware_path> <architecture>" >&2
    exit 3
fi

if [ ! -f "$FIRMWARE" ]; then
    echo "[FIRMARBITER][{tool_name}] ERROR: firmware not found: $FIRMWARE" >&2
    exit 3
fi

echo "[FIRMARBITER][{tool_name}] Starting on: $(basename "$FIRMWARE") ($ARCH)"

{run_command.replace('{firmware}', '"$FIRMWARE"').replace('{arch}', '"$ARCH"')}

EXIT_CODE=$?
echo "[FIRMARBITER][{tool_name}] Exited with code: $EXIT_CODE"
exit $EXIT_CODE
"""
...
def _generate_candidate_conf(tool_id: str, tool_name: str,
    ...
    repo_line  = (f"tool_repo = {tool_repo}\n"
                  if tool_repo else "")
    return f"""# candidate.conf — auto-generated by FIRMARBITER for {tool_name}
...
def _register_interactively(tool_dir: Path) -> dict | None:
    ...
    """
    print(f"\n[FIRMARBITER] Tool not recognised from fingerprints.")
    print(f"[FIRMARBITER] Answer a few questions to register it. (Ctrl+C to cancel)\n")
    ...
    try:
        ...
        print("\n[FIRMARBITER] Registration cancelled.")
        return None
...
def _ask_source_mode(tool_dir: Path, known_repo: str) -> tuple[str, str, str]:
    ...
    """
    print(f"\n[FIRMARBITER] How should FIRMARBITER install this tool inside Docker?\n")
    ...
    if choice == "local":
        ...
        if known_repo:
            ...
            if not repo:
                print("[FIRMARBITER] No repo URL provided. Falling back to local mode.")
...
def register_tool(tool_path: str) -> bool:
    """
    Register a tool at tool_path as a FIRMARBITER candidate.
    ...
    if not tool_dir.exists():
        print(f"[FIRMARBITER] ERROR: Path not found: {tool_dir}", file=sys.stderr)
        return False

    if not tool_dir.is_dir():
        print(f"[FIRMARBITER] ERROR: Expected a folder: {tool_dir}", file=sys.stderr)
        return False

    print(f"\n[FIRMARBITER] Inspecting: {tool_dir}")

    # Step 1 — fingerprint
    known = fingerprint_tool(tool_dir)

    if known:
        print(f"[FIRMARBITER] Recognised: {known['name']}")
    ...
    if candidate_dir.exists():
        print(f"\n[FIRMARBITER] '{registration['id']}' is already registered.")
        try:
        ...
        if not overwrite.lower().startswith("y"):
            print("[FIRMARBITER] Registration skipped.")
            return False
    ...
    print(f"\n[FIRMARBITER] Registered '{registration['name']}' successfully.")
    print(f"[FIRMARBITER] Source: {source_desc}")
    print(f"[FIRMARBITER] Folder: {candidate_dir}/")
    print(f"[FIRMARBITER] Files:")
    for f in sorted(candidate_dir.iterdir()):
        if not f.name.startswith("."):
            print(f"[FIRMARBITER]   {f.name}")
    print(f"\n[FIRMARBITER] Next: python run_firmarbiter.py --list-candidates")
    print(f"[FIRMARBITER] The Docker image will be built automatically on first run.")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python firmarbiter_register.py /path/to/tool_folder")
        sys.exit(1)
